...
for(skid in levels(donnees$skidID)){
  ...
  donnees$Puissance[indices_skid] = ma(donnees$Puissance[indices_skid], 12) # Moving average
  donnees$IndoorTemp[indices_skid] = ma(donnees$IndoorTemp[indices_skid], 12)
}
donnees = na.omit(donnees)

donnees$P_surfacique = donnees$Puissance/donnees$area * 10^6

donnees$IndoorTemp_010 = rep(0,nrow(donnees))
donnees$IndoorTemp_020 = rep(0,nrow(donnees))
donnees$IndoorTemp_030 = rep(0,nrow(donnees))
donnees$IndoorTemp_040 = rep(0,nrow(donnees))
donnees$IndoorTemp_050 = rep(0,nrow(donnees))
donnees$IndoorTemp_060 = rep(0,nrow(donnees))
...
for(skid in levels(donnees$skidID)){
  ...
  n = length(indices_skid)
  donnees$IndoorTemp_010[indices_skid] = c(rep(NA,2), donnees$IndoorTemp[indices_skid][1:(n-2)])
  donnees$IndoorTemp_020[indices_skid] = c(rep(NA,4), donnees$IndoorTemp[indices_skid][1:(n-4)])
  donnees$IndoorTemp_030[indices_skid] = c(rep(NA,6), donnees$IndoorTemp[indices_skid][1:(n-6)])
  donnees$IndoorTemp_040[indices_skid] = c(rep(NA,8), donnees$IndoorTemp[indices_skid][1:(n-8)])
  donnees$IndoorTemp_050[indices_skid] = c(rep(NA,10), donnees$IndoorTemp[indices_skid][1:(n-10)])
  donnees$IndoorTemp_060[indices_skid] = c(rep(NA,12), donnees$IndoorTemp[indices_skid][1:(n-12)])
...
write.csv(donnees, file="donnees_v2.csv", row.names = FALSE)
